...
colnames=['title','text','subject','date']
Fake = pd.read_csv('C:/DataSets/Melodee_FakeNews/Fake1.csv', names=colnames, header='infer')
Real = pd.read_csv('C:/DataSets/Melodee_FakeNews/True.csv', names=colnames, header='infer')
print(Fake.head())
print(Fake.shape)
Fake['label']='fake'
print(Fake.head())
...
## filter categories
#Fake1 = Fake[ Fake["label"].isin(['fake','real']) ][["tweetText","label"]]
Fake1 = Fake[["text","label"]]
Real1 = Real[["text","label"]]
print(Fake1.shape)

print(Real1.shape)
## rename columns

df = pd.concat([Fake1,Real1])
print(df.head())
print(df.shape)
df = df.sample(frac=1)
print(df.sample(5))
